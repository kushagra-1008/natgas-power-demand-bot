...
function parseAt(x) { return new Date(x.endsWith("Z") ? x : `${x}:00:00Z`).getTime(); }
function history(s, signal, at) {
  const target = parseAt(at) - 24 * 3600000;
  const c = s.observations.filter(o => o.signal === signal && parseAt(o.at) <= target);
  return c.length ? c.reduce((a,b) => Math.abs(parseAt(a.at)-target) <= Math.abs(parseAt(b.at)-target) ? a : b) : null;
}
function report(s, outageNote) {
  ...
  const m = {};
  for (const signal of SIGNALS) { const a=s.observations.filter(o=>o.signal===signal).at(-1); const b=a?history(s,signal,a.at):null; m[signal]=a?{...a,pct:b&&b.value?(a.value/b.value-1)*100:null,delta:b?a.value-b.value:null}:null; }
  const p=[]; if(m.gas_generation?.pct!=null)p.push(m.gas_generation.pct); if(m.total_load?.pct!=null)p.push(m.total_load.pct*.7); if(m.wind_generation?.pct!=null)p.push(-m.wind_generation.pct*.35); if(m.solar_generation?.pct!=null)p.push(-m.solar_generation.pct*.35);
  ...
  for(const x of SIGNALS){const z=m[x]; const val=z?(x==="generation_outages"&&z.delta!=null?`${z.delta>=0?"+":""}${z.delta.toFixed(0)} MWh vs 24h`:z.pct!=null?`${z.pct>=0?"+":""}${z.pct.toFixed(1)}% vs 24h`:`${z.value.toFixed(0)} MWh`):"N/A"; lines.push(`${names[x].padEnd(22)} ${val}`);}
  lines.push("",`Demand Pressure: ${v[0]}`,`Direction:        ${v[1]}`,"","Reason:",v[2],"",outageNote,"Source: U.S. Energy Information Administration (EIA)");
  return lines.join("\n");
}
...
export default async()=>{
  ...
  if(d.has("total_load")||d.has("load_forecast")){
    ...
    add(s,"total_load",latest(r,x=>x.type==="D")); add(s,"load_forecast",latest(r,x=>x.type==="DF"));
  }
  if(d.has("gas_generation")||d.has("wind_generation")||d.has("solar_generation")){
    const p=await fetchEIA("/electricity/rto/fuel-type-data/data/",{...common,"facets[respondent][]":"US48","facets[fueltype][]":["NG","WND","SUN"]},s),r=dataRows(p);
    add(s,"gas_generation",latest(r,x=>x.fueltype==="NG")); add(s,"wind_generation",latest(r,x=>x.fueltype==="WND")); add(s,"solar_generation",latest(r,x=>x.fueltype==="SUN"));
  }
  const outageNote=d.has("generation_outages")?"Outages: N/A — EIA-930 has no validated all-generator U.S. outage series.":"Outages: no all-generator U.S. outage series from EIA-930.";
  s.observations=s.observations.slice(-10000); s.lastRun=now.toISOString(); await save(s); if(d.size)await telegram(report(s,outageNote));
  return new Response(JSON.stringify({ok:true,source:"EIA",due:[...d],outage:"unavailable_from_eia930",usage:s.usage}),{headers:{"content-type":"application/json"}});
};
